...
{
    asection *text_sec = bfd_get_section_by_name(ibfd, ".text");

    asection *osec = bfd_make_section_anyway_with_flags(obfd, text_sec->name, text_sec->flags | SEC_LINKER_CREATED);
    if (osec == NULL)
    {  
        bfd_nonfatal_message (NULL, obfd, osec, NULL);
        return FALSE;
    }
    bfd_set_section_size(obfd, osec, 100);

    unsigned int alignment_power = bfd_section_alignment (ibfd, text_sec);

    bfd_set_section_vma(obfd, osec, last_vma_end);
    osec->lma = last_vma_end;
    
    printf("Zahed: last_vma_start ");
    printf_vma(last_vma_start);
    printf(" last_vma_end: "); printf_vma(last_vma_end);
    printf("\n");

    printf("Zahed: .text lma ");
    printf_vma(text_sec->lma);
    ...
    if (!bfd_copy_private_section_data (ibfd, text_sec, obfd, osec))
    {  
...
setup_section (bfd *ibfd, sec_ptr isection, void *obfdarg)
{
    ...
    if (vma > last_vma_start){
        last_vma_start = vma;
        last_vma_end  = vma + size;
        printf("Zahed: setting last vma to ");
        printf_vma(vma);
        printf(" size: %lu\n", (unsigned long)size);
    }
